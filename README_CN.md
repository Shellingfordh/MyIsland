...
- **键盘音量/亮度**：药丸自动显示变化。
- **右键菜单**：设置、Flux、托盘、快速切换、玻璃层。

## 刘海效率托盘 — 投递箱（拖拽）
这是一个**安全的拖拽投递流程**，不做系统自动化。

1. **打开投递箱**：右键菜单 → `Drop Box`，会打开投递文件夹。
2. **设置投递目标**：右键菜单 → `Drop → Downloads/Desktop/Clipboard/AirDrop`。
3. **拖拽文件到该文件夹**，药丸会在 ~1 秒内给出确认提示。

默认投递目录：`~/Downloads/MyIslandDrop`（可用 `USER_DROP_DIR` 修改）。
...
## 说明
- Flux 控制基于 `nightlight` CLI（缺失会打开安装页）。
